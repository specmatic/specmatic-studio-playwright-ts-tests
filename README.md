...
If `ENTERPRISE_ARTIFACT_URL` is set, the test run will start Specmatic
...
```bash
ENTERPRISE_ARTIFACT_URL=LATEST node utils/specmatic-studio-runtime.js resolve
```

```bash
ENTERPRISE_ARTIFACT_URL=LATEST_SNAPSHOT node utils/specmatic-studio-runtime.js resolve
```

```bash
ENTERPRISE_ARTIFACT_URL=LATEST_RELEASE node utils/specmatic-studio-runtime.js resolve
```
...
- `ENTERPRISE_ARTIFACT_URL=LATEST` resolves the newest available jar across the Specmatic snapshot and release repositories.
