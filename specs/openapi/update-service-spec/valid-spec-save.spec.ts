...
import { PRODUCT_SEARCH_BFF_SPEC_SAVE_VALID } from "../../specNames";
import {
  ...
  test(
    ...
    async ({ page, eyes }, testInfo) => {
      const configPage = new ServiceSpecConfigPage(
        ...
        testInfo,
        eyes,
        PRODUCT_SEARCH_BFF_SPEC_SAVE_VALID,
      );
      await test.step(`Go to Spec page for Service Spec: '${PRODUCT_SEARCH_BFF_SPEC_SAVE_VALID}'`, async () => {
        await configPage.gotoHomeAndOpenSidebar();
        await configPage.sideBar.selectSpec(PRODUCT_SEARCH_BFF_SPEC_SAVE_VALID);
